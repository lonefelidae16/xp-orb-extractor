...
public class XpOrbExtractorCommands {
    ...
    private static final SuggestionProvider<CommandSourceStack> DRAIN_TARGET_SUGGESTION_PROVIDER = makeEnumSuggester(XpOrbExtractorConfig.DrainTarget.class);
    private static final SuggestionProvider<CommandSourceStack> DRAIN_DEPLETION_SUGGESTION_PROVIDER = makeEnumSuggester(XpOrbExtractorConfig.DrainDepletion.class);
    private static final SuggestionProvider<CommandSourceStack> FUNCTIONAL_SUGGESTION_PROVIDER = makeEnumSuggester(FunctionalOp.class);
    ...
    private static ArgumentBuilder<CommandSourceStack, ?> buildGetCommands() {
        return Commands.literal(COMMAND_XP_ORB_GET)
                .then(Commands.literal(COMMAND_XP_ORB_DRAIN_TARGET)
                        .executes(XpOrbExtractorCommands::printDrainSource)
                )
                .then(Commands.literal(COMMAND_XP_ORB_DRAIN_AMOUNT)
                        .executes(XpOrbExtractorCommands::printDrainAmount)
                )
                .then(Commands.literal(COMMAND_XP_ORB_DEPLETION)
                        .executes(XpOrbExtractorCommands::printAllowDepletion)
                );
    }
        ...
        return Commands.literal(COMMAND_XP_ORB_ADD)
                .then(Commands.literal(COMMAND_XP_ORB_DRAIN_AMOUNT)
                        .then(Commands.argument(ARG_XP_ORB_DRAIN_AMOUNT, IntegerArgumentType.integer())
                                .executes(context -> {
                                    final int amount = IntegerArgumentType.getInteger(context, ARG_XP_ORB_DRAIN_AMOUNT);
                                    return updateDrainAmount(context, (long) CONFIG.amountToDrain + amount);
                                })
                        )
                );
    }
            ...
            dispatcher.register(
                    Commands.literal(COMMAND_XP_ORB_EXTRACTOR_SETTINGS)
                            .then(Commands.literal(COMMAND_XP_ORB_STATUS)
                                    .executes(XpOrbExtractorCommands::printDetails)
                            )
                            .then(buildGetCommands())
                            .then(buildSetCommands())
                            .then(buildAddCommands())
            );
        });
    }
    ...
    private static int printDrainSource(CommandContext<CommandSourceStack> context) {
        if (!CONFIG.bModEnabled) {
            return printDetails(context);
        }
    ...
    private static int printDetails(CommandContext<CommandSourceStack> context) {
        context.getSource().sendSuccess(() -> {
